 ...
 * This is purely preferential thinking
 * 
 * I suggest changing the motor pins if you would like to alter the inputs
 * and outputs of motors. Don't touch the motor_*() functions, These are 
...
void moveBot(float buttonState[]){
	 ...
	 */
	if (buttonState[2] <= 0.20){
		Serial.println("^");
		digitalWrite(LED, HIGH);
		motor_foward();
	}
	else if(buttonState[1] <= 0.20){
		Serial.println("v");
		digitalWrite(LED, HIGH);
		motor_backward();
	}
	else if ((buttonState[0] < 135) && (buttonState[0] > 10)) {
		Serial.println("<");
		digitalWrite(LED, HIGH);
		motor_roleft();
	}
	else if(buttonState[0] > 150){
		Serial.println(">");
		digitalWrite(LED, HIGH);
		motor_roright();
	}
